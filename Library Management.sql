...
-- Project Tasks..
-- Task 1. Create a New Book Record -- "978-1-60129-456-2', 'To Kill a Mockingbird', 'Classic', 6.00, 'yes', 'Harper
...
select issued_member_id ,count(issued_member_id )cnt from issued_status
group by issued_member_id having cnt>1;
...
join 
books b on a.issued_book_isbn = b.isbn
group by b.category;

-- Task 9: List members who registered in the last 1180 days..
select * from members where datediff(curdate(),reg_date )<1180;

-- Task 10: List Employees with Their Branch Manager's Name and their branch details..
select c.emp_name as manager_name,d.* from employees c join
...
group by a.emp_id order by count(*) desc Limit 3;